...
&& aws ec2 create-security-group --group-name lab3-secutity --description "security group lab 3" \
...
&& INST_ID=$(aws ec2 run-instances --image-id ami-0a261c0e5f51090b1 --count 1 --instance-type t2.micro --key-name lab3-key --security-group-ids $SG --user-data file://user_data.sh --query 'Instances[*].[InstanceId]' --output text) \
